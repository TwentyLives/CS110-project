    ...
    const renderUserCard = (p, showFollowButton = false, showUnfollowButton = false, showRemoveButton = false) => {
        ...
        return (
            <div key={p.id} className="friend-card">
                <Link to={`/profile/${p.id}`} className="friend-link">
                    {hasAvatar ? (
                ...
                <div className="friend-actions">
                    {showFollowButton && (
                        <button className="request-btn" onClick={() => handleFollow(p.id)}>
                            Follow
                        </button>
                    )}
                    {showUnfollowButton && (
                        <button className="request-btn" onClick={() => handleUnfollow(p.id)}>
                            Unfollow
                        </button>
                    )}
                    {showRemoveButton && (
                        <button className="request-btn remove" onClick={() => handleRemoveFollower(p.id)}>
                            Remove
                        </button>
                    )}
                </div>
        ...
        <div className="friends-page-container">
            ...
            <main className="main-content">
                ...
                <div className="friends-column">
                    <h3>Recommended</h3>
                    <div className="friends-list">
                        {recs.length ? recs.map((r) => renderUserCard(r, true)) : <p>No recommendations available.</p>}
                    </div>
                </div>
            </main>
            <Footer />
