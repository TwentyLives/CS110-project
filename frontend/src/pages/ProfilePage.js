...
function ProfilePage() {
    ...
    useEffect(() => {
        if (loggedInUser && loggedInUser.id !== userId) {
            fetch(`http://localhost:3002/following/${loggedInUser.id}`)
                .then((res) => res.json())
                .then((data) => {
    ...
    const handleFollowToggle = async () => {
        try {
            ...
            if (isFollowing) {
                // Unfollow
                const response = await fetch(`http://localhost:3002/unfollow`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ userId: loggedInUser.id, unfollowId: userId }),
                });
                if (response.ok) setIsFollowing(false);
            } else {
                // Follow
                const response = await fetch(`http://localhost:3002/follow`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ userId: loggedInUser.id, followId: userId }),
                });
    ...
    if (!profileData || !profileData.user) return <div>Profile not found.</div>;
