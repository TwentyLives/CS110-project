...
app.post("/follow", async (req, res) => {
    try {
        ...
        await db.collection("users").updateOne({ _id: new ObjectId(userId) }, { $addToSet: { following: new ObjectId(followId) } });

        // Fetch updated following list
        const updatedUser = await db.collection("users").findOne({ _id: new ObjectId(userId) }, { projection: { following: 1 } });
...
app.get("/following/:userId", async (req, res) => {
    try {
        ...
        const user = await db.collection("users").findOne({ _id: new ObjectId(userId) }, { projection: { following: 1 } });
...
app.post("/unfollow", async (req, res) => {
    try {
        ...
        await db.collection("users").updateOne({ _id: new ObjectId(userId) }, { $pull: { following: new ObjectId(unfollowId) } });
...
app.post("/remove", async (req, res) => {
    try {
        ...
        await db.collection("users").updateOne({ _id: new ObjectId(removeId) }, { $pull: { following: new ObjectId(userId) } });
...
app.post("/album/:albumId/participants", async (req, res) => {
    try {
        ...
        const result = await db
            .collection("albums")
            .updateOne({ _id: new ObjectId(albumId) }, { $addToSet: { participants: { $each: participantObjectIds } } });
...
app.get("/album/:albumId", async (req, res) => {
    try {
        ...
        const author = await db.collection("users").findOne({ _id: album.userId }, { projection: { username: 1, avatarUrl: 1 } });
...
app.get("/contests/:contestId", async (req, res) => {
    try {
        ...
        if (!ObjectId.isValid(contestId)) return res.status(400).json({ error: "Invalid contest ID" });
        ...
        if (contest.creatorId) {
            const creator = await db
                .collection("users")
                .findOne({ _id: new ObjectId(contest.creatorId) }, { projection: { username: 1, avatarUrl: 1 } });
...
app.post("/contests/:contestId/upload", upload.single("image"), async (req, res) => {
    try {
        ...
        await db.collection("contests").updateOne({ _id: new ObjectId(contestId) }, { $push: { entries: newEntry } });
...
app.post("/contests/:contestId/participants", async (req, res) => {
    try {
        ...
        await db.collection("contests").updateOne({ _id: new ObjectId(contestId) }, { $addToSet: { participantInfo: { $each: newParticipants } } });
