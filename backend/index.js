...
app.post("/contests/:contestId/entry/:userEntryId/like", async (req, res) => {
  try {
    const { contestId, userEntryId } = req.params;
    const { userId } = req.body;
    ...
    const entryIndex = contest.entries.findIndex(e => e.userId.equals(userEntryId));
    if (entryIndex === -1) return res.status(404).json({ error: "Entry not found" });

    const entry = contest.entries[entryIndex];
    let update;
    if (entry.likes.some(l => l.equals(userId))) {
      update = { $pull: { [`entries.${entryIndex}.likes`]: new ObjectId(userId) } };
    } else {
      update = { $push: { [`entries.${entryIndex}.likes`]: new ObjectId(userId) } };
    }
